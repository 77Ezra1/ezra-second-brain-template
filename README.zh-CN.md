...
> 给 AI Agent 用的本地优先外脑：Hermes、Claude Code、Codex、Cursor Agent，或者你自己的 Agent 都可以接入。

`ezra-second-brain-template` 是一套**文件系统优先**的个人外脑模板：你可以在任意 Agent 里使用它，也可以通过 Telegram、飞书、桌面聊天、命令行或你自己的 App 和 Agent 对话。系统会把原始输入、结构化笔记、待办、日报、消费、文章和主题知识沉淀到本地文件里。

简单说：你随口说一句，Agent 帮你记成可查询、可复盘、可迁移的 Markdown/JSONL。
...
- **输入足够轻**：在任意 Agent 里说一句就能记录；
- **使用足够自由**：你可以通过 Telegram / CLI / 飞书 / 桌面聊天等入口和 Agent 对话；
...
下面用 `telegram_brain_router.py` 演示聊天式命令；`--source cli` 表示这条输入来自命令行。你也可以按自己的接入方式换成其他来源标签。
